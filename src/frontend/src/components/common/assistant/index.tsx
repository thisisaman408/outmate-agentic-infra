...
export const AssistantButton: React.FC<AssistantButtonProps> = ({
  ...
  function getButtonClassName() {
    ...
    switch (type) {
      case "field":
        ...
        break;
      case "header":
        className = "mr-1 flex h-8 w-8 items-center justify-center hover:bg-accent rounded-md transition-colors";
        break;
      case "flow":
        className = "";
        break;
  ...
  return (
    <div className="relative flex items-center">
      ...
      {clicked && type === "field" && (
        <span className="text-xs text-muted-foreground ml-1">{elapsedTime}s</span>
      )}
    </div>
  );
};
