...
withEventDeliveryModes(
  ...
  async ({ page }) => {
    ...
    let outmateCount = await page
      .locator('[data-testid="outmate-0-option"]')
      .count();

    while (outmateCount === 0) {
      ...
      outmateCount = await page
        .locator('[data-testid="outmate-0-option"]')
        .count();
    }

    await page.locator('[data-testid="outmate-0-option"]').nth(0).waitFor({
      timeout: 15000,
      state: "visible",
    });

    await page.getByTestId("outmate-0-option").nth(0).click();
