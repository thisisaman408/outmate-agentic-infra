...
import { addFlowToTestOnEmptyoutmate } from "../../utils/add-flow-to-test-on-empty-outmate";
...
test(
  ...
  async ({ page }) => {
    ...
    const firstRunoutmate = await page
      .getByTestId("empty-project-description")
      .count();

    if (firstRunoutmate > 0) {
      await addFlowToTestOnEmptyoutmate(page);
    }
