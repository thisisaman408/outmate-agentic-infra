...
test(
  ...
  async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      const newEnv = { ...window.process.env, outmate_AUTO_LOGIN: "false" };
    ...
    await page.waitForSelector("text=sign in to outmate", { timeout: 30000 });
    ...
    expect(
      (
        await page.waitForSelector("text=Welcome to outmate", {
          timeout: 30000,
        })
      ).isVisible(),
    );
