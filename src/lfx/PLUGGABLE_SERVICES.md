...
```toml
[services]
database_service = "outmate.services.database.service:DatabaseService"
storage_service = "outmate.services.storage.local:LocalStorageService"
cache_service = "outmate.services.cache.service:ThreadingInMemoryCache"
```
...
4. Enable debug logging: `export outmate_LOG_LEVEL=DEBUG`

### Import Errors

**Error:** `ModuleNotFoundError: No module named 'outmate'`

**Solutions:**
1. Ensure outmate is installed: `pip install outmate`
2. Check if module path in config is correct
3. Verify the package is importable: `python -c "import outmate.services.database"`
...
See:
- `lfx.toml.example` - Example configuration file showing outmate service registration
- `src/lfx/services/` - Minimal built-in service implementations (auth, telemetry, tracing, variable, storage, etc.)
  - Auth: `lfx.services.auth.base` (BaseAuthService) and `lfx.services.auth.service` (AuthService). Use `get_auth_service()` from `lfx.services.deps`. Override with `auth_service = "outmate.services.auth.service:AuthService"` in config for full JWT/API key auth.
- `src/backend/base/outmate/services/` - Full-featured outmate services
