...
"""
        ...
        # Verify dependency analysis results
        dep_info = mock_frontend.metadata["dependencies"]
        assert dep_info["total_dependencies"] == 1  # Only outmate (os is stdlib, filtered out)

        # Check for dependencies
        package_names = [pkg["name"] for pkg in dep_info["dependencies"]]
        assert "outmate" in package_names  # outmate should be detected as external
...
from outmate.base.models.model import LCModelComponent
from outmate.field_typing import LanguageModel
...
"""
        ...
        build_component_metadata(
            mock_frontend, test_component, "outmate.components.lmstudio", "LMStudioModelComponent"
        )
        ...
        assert "outmate" in package_names  # project dependency
