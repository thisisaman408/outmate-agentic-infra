...
def _find_starter_projects_dir() -> Path:
    ...
    for parent in current.parents:
        candidate = parent / "src" / "backend" / "base" / "outmate" / "initial_setup" / "starter_projects"
        if candidate.is_dir():
            return candidate
...
"""
    ...
    def test_outmate_imports_filtered(self):
        """Components with outmate imports should NOT list outmate as a dep.

        lfx provides the outmate interfaces at runtime, so outmate/outmate_base
        should be filtered out just like lfx itself.
        """
        code = """
from outmate.custom import Component
from outmate.io import MessageTextInput
"""
        node = _make_node("LegacyComponent", code)
        packages, _ = _extract_component_requirements(node)
        assert "outmate" not in packages
        assert "outmate-base" not in packages
