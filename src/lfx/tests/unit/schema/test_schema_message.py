...
@pytest.fixture
def outmate_cache_dir(tmp_path):
    """Create a temporary outmate cache directory."""
    cache_dir = tmp_path / "outmate"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(outmate_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = outmate_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("outmate"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Outmate")
    assert message.text == "Hello, Outmate!"
...
def test_message_with_multiple_images(sample_image, outmate_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("outmate")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
