...
class TestStandaloneLFX:
    """Test LFX running standalone without outmate."""
...
class TestLFXWithOutmateConfig:
    """Test LFX with outmate configuration."""

    @pytest.fixture
    def outmate_config_dir(self, tmp_path):
        """Create a temporary outmate-style config directory."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create lfx.toml with outmate services
...
"""
    ...
    def test_config_overrides_defaults(self, clean_manager, outmate_config_dir):
        """Test that config file overrides default services."""
        # Discover plugins from config
        clean_manager.discover_plugins(outmate_config_dir)
    ...
    def test_multiple_services_from_config(self, clean_manager, outmate_config_dir):
