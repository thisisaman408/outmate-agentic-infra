...
async def emit_vertex_build_event(
    *,
    ...
    try:
        ...
        from outmate.services.event_manager import webhook_event_manager
        ...
        pass  # Outmate not available (standalone lfx usage)
...
async def log_transaction(
    ...
    Uses the pluggable TransactionService to log transactions. When running within Outmate
...
async def log_vertex_build(
    *,
    ...
    When running within Outmate it will use Outmate database service to persist the build.
    When running standalone (lfx only), it will only log debug messages.
    """
    try:
        # Try to use Outmate's services if available (when running within Outmate
        try:
            from outmate.services.deps import get_db_service as outmate_get_db_service
            from outmate.services.deps import get_settings_service as outmate_get_settings_service

            settings_service = outmate_get_settings_service()
            ...
            from outmate.services.database.models.vertex_builds.crud import (
                log_vertex_build as crud_log_vertex_build,
            )
            from outmate.services.database.models.vertex_builds.model import VertexBuildBase
            ...
            db_service = outmate_get_db_service()
            ...
            # Fallback for standalone lfx usage (without Outmate
            ...
            # Log basic vertex build info - concrete implementation is in Outmate
