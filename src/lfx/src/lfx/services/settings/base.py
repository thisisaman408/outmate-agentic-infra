...
class Settings(BaseSettings):
    # Define the default outmate_DIR
    config_dir: str | None = None
    # Define if outmate db should be saved in config dir or
    # in the outmate directory
    save_db_in_config_dir: bool = False
    """Define if outmate database should be saved in outmate_CONFIG_DIR or in the outmate directory
    (i.e. in the package directory)."""

    knowledge_bases_dir: str | None = "~/.outmate/knowledge_bases"
    """The directory to store knowledge bases."""

    dev: bool = False
    """If True, outmate will run in development mode."""
    database_url: str | None = None
    """Database URL for outmate. If not provided, outmate will use a SQLite database.
    ...
    """If True, outmate will retry to connect to the database if it fails."""
    pool_size: int = 20
    ...
    If not provided, a hash of the database URL will be used. Useful when multiple outmate
    ...
    Controlled by outmate_USE_NOOP_DATABASE env variable."""
    ...
    """If set to True, outmate will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which outmate will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.outmate.store"
    download_webhook_url: str | None = "https://api.outmate.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.outmate.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"

    storage_type: str = "local"
    """Storage type for file storage. Defaults to 'local'. Supports 'local' and 's3'."""
    object_storage_bucket_name: str | None = "outmate-bucket"
    """Object storage bucket name for file storage. Defaults to 'outmate-bucket'."""
    ...
    with the same name in case outmate fails to retrieve the variable value."""
    ...
    user_agent: str = "outmate"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, outmate will not serve the frontend."""
    ...
    """If set to True, outmate will not track telemetry."""
    telemetry_base_url: str = "https://outmate.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, outmate will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, outmate will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "localhost"
    """The host on which outmate will run."""
    port: int = 7860
    """The port on which outmate will run."""
    ...
    """The log level for outmate."""
    log_file: str | None = "logs/outmate.log"
    """The path to log file for outmate."""
    ...
    """If set to True, outmate will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, outmate will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which outmate will auto save flows."""
    ...
    """If set to False, outmate will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, outmate will not send progress notifications in the MCP server."""
    ...
    """If set to False, outmate will not start the MCP Composer service."""
    ...
    """If set to True, outmate will start the agentic MCP server that provides tools for
    flow/component operations, template search, and graph visualization."""

    # Developer API
    developer_api_enabled: bool = False
    """If set to True, outmate will enable developer API endpoints for advanced debugging and introspection."""
    ...
    """If set to True, outmate will only partially load components at startup and fully load them on demand.
    ...
    """If set to True, outmate will create starter projects. If False, skips all starter project setup.
    ...
    """If set to True, outmate will update starter projects."""

    # SSRF Protection
    ssrf_protection_enabled: bool = False
    """If set to True, outmate will enable SSRF (Server-Side Request Forgery) protection.
    ...
    def validate_runtime_port(cls, value):
        ...
        Kubernetes auto-creates env vars like outmate_RUNTIME_PORT=tcp://<ip>:<port>
        for services, which collides with the outmate_ env prefix. Extract the port
    ...
    def set_variables_to_get_from_environment(cls, value):
        ...
        if os.getenv("outmate_AGENTIC_EXPERIENCE", "true").lower() == "true":
    ...
    def set_outmate_dir(cls, value):
        if not value:
            ...
            # Create a .outmate directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        if outmate_database_url := os.getenv("outmate_DATABASE_URL"):
            value = outmate_database_url
            logger.debug("Using outmate_DATABASE_URL env variable")
        else:
            # Originally, we used sqlite:///./outmate.db
            ...
            from lfx.utils.version import is_pre_release as outmate_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = outmate_is_pre_release(version)

            if info.data["save_db_in_config_dir"]:
                database_dir = info.data["config_dir"]
            else:
                # Use outmate package path, not lfx, for backwards compatibility
                try:
                    import outmate

                    database_dir = Path(outmate.__file__).parent.resolve()
                except ImportError:
                    database_dir = Path(__file__).parent.parent.parent.resolve()

            pre_db_file_name = "outmate-pre.db"
            db_file_name = "outmate.db"
    ...
    def set_components_path(cls, value):
        ...
        If the `outmate_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
        appended to the provided list if not already present. If the input list is empty or missing, it is
        set to an empty list.
        """
        if os.getenv("outmate_COMPONENTS_PATH"):
            logger.debug("Adding outmate_COMPONENTS_PATH to components_path")
            outmate_component_path = os.getenv("outmate_COMPONENTS_PATH")
            if Path(outmate_component_path).exists() and outmate_component_path not in value:
                if isinstance(outmate_component_path, list):
                    for path in outmate_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {outmate_component_path} to components_path")
                elif outmate_component_path not in value:
                    value.append(outmate_component_path)
                    logger.debug(f"Appending {outmate_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="outmate_")
