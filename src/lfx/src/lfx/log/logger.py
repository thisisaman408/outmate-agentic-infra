"""Logging configuration for Outmate using structlog."""
...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable OutmateLOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("OutmateLOG_RETRIEVER_BUFFER_SIZE", "0")
...
def configure(
    *,
    ...
    if os.getenv("OutmateLOG_LEVEL", "").upper() in VALID_LOG_LEVELS and log_level is None:
        log_level = os.getenv("OutmateLOG_LEVEL")

    log_level_str = os.getenv("OutmateLOG_LEVEL", "ERROR")
    ...
    if log_file is None:
        env_log_file = os.getenv("OUTMATE_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("OutmateLOG_ENV", "")

    # Get log format from env if not provided
    if log_format is None:
        log_format = os.getenv("OutmateLOG_FORMAT")
    ...
    # Add callsite information only when OutmateDEV is set
    if DEV:
        processors.append(
    ...
    if log_env.lower() == "container" or log_env.lower() == "container_json":
        ...
        log_stdout_pretty = os.getenv("OutmatePRETTY_LOGS", "true").lower() == "true"
    ...
    if log_file:
        if not log_file.parent.exists():
            ...
            log_file = cache_dir / "outmate.log"
