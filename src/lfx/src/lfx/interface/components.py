...
def _get_cache_path() -> Path:
    ...
    cache_dir = Path(user_cache_dir("lfx", "outmate"))
...
def _save_generated_index(modules_dict: dict) -> None:
    ...
    try:
        ...
        outmate_version = version("outmate")

        # Build index structure
        index = {
            "version": outmate_version,
            "metadata": {
...
async def _send_telemetry(
    ...
    try:
        ...
        from outmate.services.telemetry.schema import ComponentIndexPayload
...
async def _load_components_dynamically(
    ...
    try:
        ...
        await logger.aerror(f"Failed to import outmate.components package: {e}", exc_info=True)
        return modules_dict
...
async def import_outmate_components(
    settings_service: Optional["SettingsService"] = None,
    telemetry_service: Any | None = None,
) -> dict[str, dict[str, Any]]:
    """Asynchronously discovers and loads all built-in outmate_components.
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Outmate
    ...
    if component_cache.all_types_dict is None:
        ...
        outmate_components = await import_outmate_components(settings_service, telemetry_service)
