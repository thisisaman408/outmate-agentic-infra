...
_OUTMATE_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import outmate  # noqa: F401

    _OUTMATE_IS_INSTALLED = True
...
def validate_code(code):
    ...
    # Evaluate the function definition with Outmate context
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            code_obj = compile(ast.Module(body=[node], type_ignores=[]), "<string>", "exec")
            try:
                # Create execution context with common Outmate imports
                exec_globals = _create_outmate_execution_context()
...
def _create_outmate_execution_context():
    """Create execution context with common Outmate imports."""
    context = {}

    # Import common Outmate types that are used in templates
    try:
...
def create_class(code, class_name):
    ...
    code = code.replace("from outmate import CustomComponent", "from outmate.custom import CustomComponent")
    code = code.replace(
        "from outmate.interface.custom.custom_component import CustomComponent",
        "from outmate.custom import CustomComponent",
    )
...
def prepare_global_scope(module):
    ...
    for node in import_froms:
        ...
        # If original module starts with outmate also try lfx equivalent
        if node.module and node.module.startswith("outmate"):
            lfx_module_name = node.module.replace("outmate", "lfx", 1)
...
def get_default_imports(code_string):
    ...
    outmate_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, outmate_imports)
    outmate_module = importlib.import_module("lfx.field_typing")
    default_imports.update({name: getattr(outmate_module, name) for name in necessary_imports})
