# lfx - outmate Executor

lfx is a command-line tool for running outmate workflows. It provides two main commands: `serve` and `run`.
...
git clone https://github.com/outmate-ai/outmate
cd outmate/src/lfx
...
lfx supports a pluggable service architecture that allows you to customize and extend its behavior. You can replace built-in services (storage, telemetry, tracing, etc.) with your own implementations or use outmate's full-featured services.
...
- Using full-featured outmate services in lfx
...
Serve a outmate workflow as a REST API.

**Important:** You must set the `outmate_API_KEY` environment variable before running the serve command.

```bash
export outmate_API_KEY=your-secret-key
...
Execute a outmate workflow and get results immediately.
...
```python
"""A simple agent flow example for outmate.

This script demonstrates how to set up a conversational agent using outmate's
