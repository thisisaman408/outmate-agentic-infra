"""Outmate API Locust Load Testing File.

Comprehensive load testing for Outmate API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - OUTMATE_HOST: Base URL for the Outmate server (default: http://localhost:7860)
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://docs.outmate.org for me? Later, could you calculate 1390 / 192 ?",
}
...
ERROR_LOG_FILE = None
OUTMATE_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, OUTMATE_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"outmate_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("outmate_load_test_errors")
    ...
    # Try to capture Outmate logs
    outmate_log_paths = ["outmate.log", "logs/outmate.log", "../../../outmate.log", "../../../../outmate.log"]

    for log_path in outmate_log_paths:
        if Path(log_path).exists():
            OUTMATE_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if OUTMATE_LOG_FILE:
        print(f"   • Outmate logs: {OUTMATE_LOG_FILE}")
    else:
        print("   • Outmate logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"outmate_load_test_error_summary_{timestamp}.json"
...
def capture_outmate_logs():
    """Capture recent Outmate logs if available."""
    if not OUTMATE_LOG_FILE or not Path(OUTMATE_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Outmate log
        with open(OUTMATE_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"outmate_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Outmate server logs captured during load test\n")
            f.write(f"# Original log file: {OUTMATE_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Outmate logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Outmate logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"OUTMATE API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
    ...
    # Capture Outmate logs
    capture_outmate_logs()
...
class BaseOutmateUser(FastHttpUser):
    """Base class for all Outmate API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("OUTMATE_HOST", "http://localhost:7860")
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_outmate_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_outmate_test.py first.")
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Outmate API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseOutmateUser):
...
class AggressiveUser(BaseOutmateUser):
...
class SustainedLoadUser(BaseOutmateUser):
...
class TailLatencyHunter(BaseOutmateUser):
...
class ScalabilityTestUser(BaseOutmateUser):
...
class BurstUser(BaseOutmateUser):
