#!/usr/bin/env python3
"""Example Outmate Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Outmate load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Outmate Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Outmate Load Testing Example Workflow")
    ...
    setup_script = script_dir / "outmate_setup_test.py"
    runner_script = script_dir / "outmate_run_load_test.py"
    ...
    try:
        ...
        try:
            ...
            env = os.environ.copy()
            env["OUTMATE_HOST"] = creds["host"]
            env["API_KEY"] = creds["api_key"]
            env["FLOW_ID"] = creds["flow_id"]

            print("   🔧 Setting environment variables:")
            print(f"      OUTMATE_HOST={creds['host']}")
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-outmate",
                "--html",
                "outmate_load_test_report.html",
                "--csv",
                "outmate_load_test_results",
            ],
        ...
        print("   python setup_outmate_test.py --interactive")
        print()
        ...
        print("   - outmate_load_test_report.html (detailed HTML report)")
        print("   - outmate_load_test_results_*.csv (CSV data files)")
        print("   - outmate_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - outmate_load_test_error_summary_*.json (error analysis)")
        print("   - outmate_server_logs_during_test_*.log (Outmate server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Outmate UI")
        ...
        if args.auto:
            ...
            print("   - outmate_load_test_report.html")
            print("   - outmate_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "outmate_load_test_report.html",
                    "outmate_load_test_results_failures.csv",
                    "outmate_load_test_results_stats.csv",
                    "outmate_load_test_results_stats_history.csv",
                    "outmate_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("outmate_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("outmate_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("outmate_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Outmate load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open outmate_load_test_report.html in your browser for detailed analysis")
        print("   • Check outmate_load_test_results_*.csv for raw data")
        print("   • Review outmate_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze outmate_load_test_error_summary_*.json for error patterns")
        print("   • Examine outmate_server_logs_during_test_*.log for server-side issues")
