#!/usr/bin/env python3
"""Outmate Load Test Setup CLI

This script sets up a complete Outmate test environment by:
1. Starting Outmate (optional)
...
Usage:
    python setup_outmate_test.py --help
    python setup_outmate_test.py --interactive
    python setup_outmate_test.py --flow "Basic Prompting"
    python setup_outmate_test.py --list-flows
"""
...
async def get_starter_projects_from_api(host: str, access_token: str) -> list[dict]:
    """Get starter projects from Outmate API."""
    import httpx
...
async def list_available_flows(host: str, access_token: str) -> list[tuple[str, str, str]]:
    """List all available starter project flows from Outmate API.

    Args:
        host: Outmate host URL
...
async def setup_outmate_environment(host: str, flow_name: str | None = None, interactive: bool = False) -> dict:
    """Set up complete Outmate environment with real starter project flows."""
    try:
        import httpx
    except ImportError:
    ...
    # Configuration - use default Outmate credentials
    username = "outmate"
    password = "outmate"
    ...
    async with httpx.AsyncClient(base_url=host, timeout=60.0) as client:
        ...
        try:
            ...
            # Remove the id to let Outmate generate a new one
...
def print_setup_results(setup_state: dict):
    ...
    print(f"export OUTMATE_HOST='{setup_state['host']}'")
    ...
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-outmate --headless --users 25 --duration 120"
    )
    print()

    print("# Generate HTML report:")
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-outmate --headless --users 50 --duration 180 --html report.html"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up Outmate load test environment with real starter project flows",
...
Examples:
  ...
  # Use specific flow
  python setup_outmate_test.py --flow "Memory Chatbot"

  # List available flows
  python setup_outmate_test.py --list-flows

  # Setup with custom host
  python setup_outmate_test.py --host http://localhost:8000 --interactive

  # Save credentials to file
  python setup_outmate_test.py --interactive --save-credentials test_creds.json
        """,
    )
        ...
        async def list_flows_only():
            ...
            async with httpx.AsyncClient(base_url=args.host, timeout=30.0) as client:
                ...
                try:
                    health_response = await client.get("/health")
                    if health_response.status_code != 200:
                        raise Exception(f"Outmate not available at {args.host}")
                except Exception as e:
                    print(f"❌ Cannot connect to Outmate at {args.host}: {e}")
                    sys.exit(1)
                ...
                try:
                    ...
                    print(f"❌ Authentication failed: {e}")
                    print("Make sure Outmate is running with default credentials (outmate/outmate)")
                    sys.exit(1)
    ...
    try:
        ...
        setup_state = asyncio.run(
            setup_outmate_environment(host=args.host, flow_name=args.flow, interactive=args.interactive)
        )
