# Outmate Load Testing

This directory contains comprehensive load testing tools for both Outmate and LFX APIs using Locust.

## 🔧 **Two Testing Systems**

### **Outmate API Testing** (Enhanced System)

- Files: `outmate_*.py`
- Tests the main Outmate application API
...
- **Enhanced Error Logging**: Detailed connection error analysis and Outmate log capture
- **Easy Setup**: One-command execution with automatic Outmate startup
...
For testing against a remote Outmate instance:

### Setup for Remote Testing

```bash
# Using Makefile (recommended)
make load_test_remote_setup OUTMATE_HOST="https://your-remote-instance.com"
make load_test_remote_run OUTMATE_HOST="https://your-remote-instance.com"

# Or using Python scripts directly
python outmate_setup_test.py --host https://your-remote-instance.com --interactive
python outmate_run_load_test.py --host https://your-remote-instance.com --no-start-outmate --headless --users 10 --duration 120
...
- **Always use `--no-start-outmate`** when testing remote instances
...
Choose and set up a real Outmate starter project for testing:

```bash
# Interactive flow selection
python outmate_setup_test.py --interactive

# Use specific flow
python outmate_setup_test.py --flow "Memory Chatbot"

# List available flows
python outmate_setup_test.py --list-flows
```

This will:

- Use default Outmate credentials (outmate/outmate)
- Generate API keys
...
python outmate_run_load_test.py

# Headless mode with 20 users for 2 minutes
python outmate_run_load_test.py --headless --users 20 --duration 120

# Use predefined load shape
python outmate_run_load_test.py --shape ramp100 --headless --users 100 --duration 180
```
...
python outmate_setup_test.py --interactive --save-credentials my_test_creds.json

# Test against existing remote Outmate instance
python outmate_run_load_test.py --host https://your-remote-instance.com --no-start-outmate

# Save results to CSV and HTML
python outmate_run_load_test.py --headless --csv results --html report.html --users 50 --duration 300

# Direct Locust usage (after setup)
export API_KEY="your-api-key-from-setup"
export FLOW_ID="your-flow-id-from-setup"
locust -f outmate_locustfile.py --host http://localhost:7860

# Distributed testing (master)
locust -f outmate_locustfile.py --host http://localhost:7860 --master

# Distributed testing (worker)
locust -f outmate_locustfile.py --host http://localhost:7860 --worker --master-host=localhost
```
...
- `OUTMATE_HOST`: Base URL for Outmate server (default: http://localhost:7860)
- `SHAPE`: Load test shape (ramp100, stepramp)
- `REQUEST_TIMEOUT`: Request timeout in seconds (default: 30.0)

## Architecture

### Setup Process (`outmate_setup_test.py`)

1. **Health Check**: Verify Outmate is running
2. **Flow Selection**: Choose from 40+ real starter project flows
3. **Authentication**: Login with default credentials (outmate/outmate)
...
Instead of simple test flows, the system uses real Outmate starter projects:
...
1. **Setup Failed**: Ensure Outmate is accessible and not in read-only mode
2. **Authentication Errors**: Verify default credentials (outmate/outmate) are enabled
...
1. Run Outmate manually with `--log-level debug`
2. Check the Outmate logs for detailed error information
...
1. Start Outmate: `python -m outmate run --auto-login`
...
1. Inherit from `BaseOutmateUser`
...
```bash
python outmate_run_load_test.py --headless --users 10 --duration 60
```

### Stress Test

```bash
python outmate_run_load_test.py --shape ramp100 --headless --users 100 --duration 300
```

### Performance Profiling

```bash
python outmate_run_load_test.py --shape stepramp --headless --csv profile_results
```

### Production Readiness Test

```bash
python outmate_run_load_test.py --users 50 --duration 600 --csv production_test --html production_report.html
```
...
python outmate_run_load_test.py --headless --users 25 --duration 120 --html detailed_report.html

# Combined CSV + HTML reporting
python outmate_run_load_test.py --headless --users 100 --duration 300 --csv data --html analysis.html --shape ramp100

# Quick test with report
python outmate_setup_test.py --flow "Memory Chatbot"
python outmate_run_load_test.py --headless --users 10 --duration 60 --html quick_test.html
```
