...
import httpx
import pytest
from outmate.utils.version import (
...
class TestGetVersionInfo:
    ...
    @patch("outmate.utils.version.metadata")
    def test_get_version_info_outmate_package(self, mock_metadata):
        """Test getting version info for outmate package."""
        ...
        assert result["package"] == "Outmate"

    @patch("outmate.utils.version.metadata")
    def test_get_version_info_outmate_base_package(self, mock_metadata):
        """Test getting version info for outmate-base package."""
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Outmate Base"
    ...
    def test_get_version_info_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["outmate", "outmate-base"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "outmate-nightly":
                return "1.0.0.dev456"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Outmate Nightly"
    ...
    def test_get_version_info_base_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["outmate", "outmate-base", "outmate-nightly"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "outmate-base-nightly":
                return "1.0.0.a1"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Outmate Base Nightly"
...
class TestFetchLatestVersion:
    ...
    @patch("outmate.utils.version.httpx")
...
class TestGetVersionInfoFunction:
    ...
    @patch("outmate.utils.version.VERSION_INFO")
    def test_get_version_info_returns_version_info(self, mock_version_info):
        """Test that get_version_info returns VERSION_INFO."""
        mock_version_info = {"version": "1.0.0", "main_version": "1.0.0", "package": "Outmate"}

        with patch("outmate.utils.version.VERSION_INFO", mock_version_info):
