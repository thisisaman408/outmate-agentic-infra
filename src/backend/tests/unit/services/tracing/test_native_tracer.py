...
class TestIsEnabled:
    def test_enabled_by_default(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OutmateNATIVE_TRACING", None)
    ...
    def test_disabled_by_env_var(self, value):
        with patch.dict(os.environ, {"OutmateNATIVE_TRACING": value}):
    ...
    def test_ready_property_reflects_is_enabled(self):
        with patch.dict(os.environ, {"OutmateNATIVE_TRACING": "false"}):
            tracer = _make_tracer()
            assert tracer.ready is False

        with patch.dict(os.environ, {"OutmateNATIVE_TRACING": "true"}):
            tracer = _make_tracer()
...
class TestEnd:
    ...
    def test_end_logs_error_when_no_event_loop(self):
        ...
        with patch("outmate.services.tracing.native.logger") as mock_logger:
...
class TestFlushToDatabase:
    ...
    async def test_flush_invalid_flow_id_logs_error_and_continues(self):
        ...
        with (
            patch("outmate.services.tracing.native.logger") as mock_logger,
    ...
    async def test_flush_error_status_when_span_has_error(self):
        ...
        from outmate.services.database.models.traces.model import TraceTable
