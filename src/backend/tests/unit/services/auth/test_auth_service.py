...
from outmate.services.auth.exceptions import (
    InactiveUserError,
    InvalidTokenError,
    TokenExpiredError,
)
...
async def test_get_current_user_from_access_token_returns_active_user(auth_service: AuthService):
    ...
    with patch("outmate.services.auth.service.get_user_by_id", new=AsyncMock(return_value=fake_user)) as mock_get_user:
...
async def test_get_current_user_from_access_token_requires_active_user(auth_service: AuthService):
    ...
    with (
        patch("outmate.services.auth.service.get_user_by_id", new=AsyncMock(return_value=inactive_user)),
...
async def test_create_user_tokens_updates_last_login(auth_service: AuthService):
    ...
    with patch("outmate.services.auth.service.update_user_last_login_at", new=AsyncMock()) as mock_update:
...
async def test_create_refresh_token_valid(auth_service: AuthService):
    ...
    with patch("outmate.services.auth.service.get_user_by_id", new=AsyncMock(return_value=fake_user)):
...
async def test_create_refresh_token_user_not_found(auth_service: AuthService):
    ...
    with (
        patch("outmate.services.auth.service.get_user_by_id", new=AsyncMock(return_value=None)),
...
async def test_authenticate_user_success(auth_service: AuthService):
    ...
    with patch("outmate.services.auth.service.get_user_by_username", new=AsyncMock(return_value=user)):
...
async def test_authenticate_user_not_found(auth_service: AuthService):
    ...
    with patch("outmate.services.auth.service.get_user_by_username", new=AsyncMock(return_value=None)):
...
async def test_authenticate_user_inactive_never_logged_in(auth_service: AuthService):
    ...
    with (
        patch("outmate.services.auth.service.get_user_by_username", new=AsyncMock(return_value=user)),
        pytest.raises(HTTPException) as exc,
    ):
