...
import pytest
from outmate.services.auth.mcp_encryption import is_encrypted
from outmate.services.auth.service import AuthService
from outmate.services.auth.utils import decrypt_api_key, encrypt_api_key
from lfx.services.settings.auth import AuthSettings
from pydantic import SecretStr


@pytest.fixture
def outmate_auth_service(tmp_path):
    """Use Outmate AuthService for encrypt/decrypt so tests get real Fernet behavior."""
...
def use_outmate_auth_for_encryption(outmate_auth_service):
    """Ensure utils use Outmate AuthService (real encrypt/decrypt), not LFX stub."""
    with patch("outmate.services.auth.utils.get_auth_service", return_value=outmate_auth_service):
        yield
