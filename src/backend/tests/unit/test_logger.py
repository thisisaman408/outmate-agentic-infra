...
class TestConfigure:
    ...
    @patch.dict(os.environ, {"OutmateLOG_LEVEL": "WARNING"})
    def test_configure_env_variable_override(self):
        """Test configure() respects OutmateLOG_LEVEL environment variable."""
    ...
    @patch.dict(os.environ, {"OUTMATE_LOG_FILE": "/tmp/test.log"})  # noqa: S108
    def test_configure_env_log_file_override(self):
        """Test configure() respects OUTMATE_LOG_FILE environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"OutmateLOG_ENV": "container"})
    def test_configure_env_log_env_override(self):
        """Test configure() respects OutmateLOG_ENV environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"OutmateLOG_FORMAT": "custom"})
    def test_configure_env_log_format_override(self):
        """Test configure() respects OutmateLOG_FORMAT environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"OutmatePRETTY_LOGS": "false"})
    def test_configure_env_pretty_logs_disabled(self):
        """Test configure() respects OutmatePRETTY_LOGS=false."""
        configure()
...
def test_init_with_env_variable():
    with patch.dict(os.environ, {"OutmateLOG_RETRIEVER_BUFFER_SIZE": "100"}):
        buffer = SizedLogBuffer()
