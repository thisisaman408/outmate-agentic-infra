...
from outmate.services.variable.constants import CREDENTIAL_TYPE
...
def migrate_module():
    ...
    repo_root = test_file.parents[5]  # Goes to outmate repo root
...
class TestDatabaseMigrationUnit:
    ...
    def test_migrate_user_store_api_key(self, migrate_module, sqlite_db, old_key, new_key):
        ...
        user_id = str(uuid4())
        original_value = "outmate-store-api-key"
...
class TestMigrationWithRealDatabase:
    """Integration tests using real Outmate database fixtures."""
...
class TestMigrationCompatibility:
    """Test that migration script is compatible with Outmate's encryption."""

    def test_script_encryption_matches_outmate(self, migrate_module):
        """Verify migration script produces same results as Outmate's auth utils."""
        from outmate.services.auth import utils as auth_utils
        ...
        # Encrypt with Outmate
        outmate_encrypted = auth_utils.encrypt_api_key(plaintext, settings_service)

        # Decrypt with migration script
        script_decrypted = migrate_module.decrypt_with_key(outmate_encrypted, secret_key)
        ...
        # Decrypt with Outmate
        outmate_decrypted = auth_utils.decrypt_api_key(script_encrypted, settings_service)
        assert outmate_decrypted == plaintext
