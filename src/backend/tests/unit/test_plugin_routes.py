...
Ensures that plugins loaded via the outmate.plugins entry-point group
cannot overwrite or shadow existing Outmate routes.
"""
...
from outmate.plugin_routes import (
    _get_route_keys,
    _PluginAppWrapper,
    load_plugin_routes,
)
...
class TestLoadPluginRoutes:
    ...
    def test_no_crash_when_no_plugins(self):
        ...
        with patch("outmate.plugin_routes.entry_points", return_value=[]):
    ...
    def test_plugin_that_registers_route_is_loaded(self):
        ...
        with patch("outmate.plugin_routes.entry_points", return_value=[ep]):
            load_plugin_routes(app)
