...
def max_file_size_upload_fixture(monkeypatch):
    monkeypatch.setenv("OutmateMAX_FILE_SIZE_UPLOAD", "1")
    yield
    monkeypatch.undo()
...
def max_file_size_upload_10mb_fixture(monkeypatch):
    monkeypatch.setenv("OutmateMAX_FILE_SIZE_UPLOAD", "10")
    yield
    monkeypatch.undo()
...
async def files_client_fixture(
    ...
    if "noclient" in request.keywords:
        ...
        def init_app():
            ...
            monkeypatch.setenv("OutmateAUTO_LOGIN", "false")
...
async def setup_profile_pictures(monkeypatch):
    ...
    This fixture must run before files_client to set OUTMATE_CONFIG_DIR
...
async def test_profile_pictures_with_s3_storage(setup_profile_pictures, files_client, monkeypatch):  # noqa: ARG001
    ...
    monkeypatch.setenv("OutmateSTORAGE_TYPE", "s3")
