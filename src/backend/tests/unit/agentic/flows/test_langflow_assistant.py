"""Tests for OutmateAssistant flow.
...
from outmate.agentic.flows.outmate_assistant import (
    ASSISTANT_PROMPT,
    _build_model_config,
    get_graph,
)
...
class TestGetGraph:
    ...
    def test_should_create_graph_with_default_provider(self):
        ...
        with (
            patch("outmate.agentic.flows.outmate_assistant.ChatInput") as mock_chat_input,
            patch("outmate.agentic.flows.outmate_assistant.ChatOutput") as mock_chat_output,
            patch("outmate.agentic.flows.outmate_assistant.LanguageModelComponent") as mock_llm,
            patch("outmate.agentic.flows.outmate_assistant.Graph"),
        ):
    ...
    def test_should_use_provided_provider_and_model(self):
        ...
        with (
            patch("outmate.agentic.flows.outmate_assistant.ChatInput"),
            patch("outmate.agentic.flows.outmate_assistant.ChatOutput"),
            patch("outmate.agentic.flows.outmate_assistant.LanguageModelComponent") as mock_llm,
            patch("outmate.agentic.flows.outmate_assistant.Graph"),
        ):
            mock_llm_instance = MagicMock()
    ...
    def test_should_configure_chat_input(self):
        ...
        with (
            patch("outmate.agentic.flows.outmate_assistant.ChatInput") as mock_chat_input,
            patch("outmate.agentic.flows.outmate_assistant.ChatOutput"),
            patch("outmate.agentic.flows.outmate_assistant.LanguageModelComponent"),
            patch("outmate.agentic.flows.outmate_assistant.Graph"),
        ):
    ...
    def test_should_create_graph_with_start_and_end(self):
        ...
        with (
            ...
            patch("outmate.agentic.flows.outmate_assistant.ChatOutput") as mock_chat_output,
            patch("outmate.agentic.flows.outmate_assistant.LanguageModelComponent"),
            patch("outmate.agentic.flows.outmate_assistant.Graph") as mock_graph_class,
        ):
...
class TestAssistantPrompt:
    ...
    def test_should_contain_outmate_references(self):
        """Should contain Outmate documentation references."""
        assert "outmate" in ASSISTANT_PROMPT.lower()
        assert "docs.outmate.org" in ASSISTANT_PROMPT

    def test_should_contain_code_requirements(self):
        """Should contain code requirements for components."""
        assert "lfx.custom" in ASSISTANT_PROMPT or "outmate.custom" in ASSISTANT_PROMPT
