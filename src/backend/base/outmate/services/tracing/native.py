...
execution traces directly in Outmate's database, enabling the Trace View
...
from outmate.serialization.serialization import serialize
from outmate.services.database.models.traces.model import SpanStatus, SpanType
from outmate.services.tracing.base import BaseTracer
...
if TYPE_CHECKING:
    ...
    from outmate.services.tracing.schema import Log

OutmateSPAN_NAMESPACE = UUID("a3e1c2d4-5b6f-7890-abcd-ef1234567890")
...
class NativeTracer(BaseTracer):
    """Tracer that stores execution traces in Outmate's database.

    This tracer captures:
    - Component-level traces (via add_trace/end_trace)
    - LangChain-level traces (via get_langchain_callback)

    Enabled by default. Disable with OutmateNATIVE_TRACING=false if needed.
    """
    ...
    def _is_enabled() -> bool:
        ...
        return os.getenv("OutmateNATIVE_TRACING", "true").lower() not in ("false", "0", "no")
    ...
    async def _flush_to_database(self, error: Exception | None = None) -> None:
        ...
        try:
            ...
            from outmate.services.database.models.traces.model import SpanTable, TraceTable

            try:
                flow_uuid = UUID_(self.flow_id)
            except (ValueError, TypeError):
                # Deterministic fallback so malformed flow_ids don't silently discard trace data.
                flow_uuid = uuid5(OutmateSPAN_NAMESPACE, f"invalid-flow-id:{self.flow_id}")
                logger.error(
            ...
            from outmate.services.tracing.formatting import safe_int_tokens
            ...
            async with session_scope() as session:
                ...
                for span_data in self.completed_spans:
                    try:
                        ...
                        span_uuid = uuid5(OutmateSPAN_NAMESPACE, f"{self.trace_id}-{span_data['id']}")
                    ...
                    if span_data.get("parent_span_id"):
                        ...
                        if isinstance(parent_id, UUID_):
                            ...
                            try:
                                ...
                                parent_uuid = uuid5(OutmateSPAN_NAMESPACE, f"{self.trace_id}-{parent_id}")
    ...
    def get_langchain_callback(self) -> BaseCallbackHandler | None:
        ...
        from outmate.services.tracing.native_callback import NativeCallbackHandler
        from outmate.services.tracing.service import component_context_var
        ...
        if component_context:
            ...
            parent_span_id = uuid5(OutmateSPAN_NAMESPACE, f"{self.trace_id}-{component_id}")
        elif self._current_component_id:
            # Fallback for edge cases where component context might not be set
            parent_span_id = uuid5(OutmateSPAN_NAMESPACE, f"{self.trace_id}-{self._current_component_id}")
    ...
    def end_langchain_span(
        ...
        # Default to chat since most LLM usage in Outmate is conversational
    ...
    def _map_trace_type(trace_type: str) -> SpanType:
        """Normalise Outmate's string trace types to the SpanType enum, defaulting to CHAIN for unknown values."""
