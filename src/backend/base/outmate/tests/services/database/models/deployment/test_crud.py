...
DEPLOYMENT_CLASS = "outmate.services.database.models.deployment.crud.Deployment"
...
async def test_create_deployment_integrity_error_raises_value_error():
    ...
    with (
        ...
        patch("outmate.services.database.models.deployment.crud.logger") as mock_logger,
    ):
...
async def test_delete_by_resource_key_none_rowcount_logs_error():
    ...
    with patch("outmate.services.database.models.deployment.crud.logger") as mock_logger:
        mock_logger.aerror = AsyncMock()
        count = await delete_deployment_by_resource_key(
            db,
            user_id=uuid4(),
