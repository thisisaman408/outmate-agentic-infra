...
from outmate.api import health_check_router, log_router
from outmate.api.router import router
from outmate.api.v1.mcp_projects import init_mcp_servers
...
from outmate.middleware import ContentSizeLimitMiddleware
from outmate.plugin_routes import load_plugin_routes
from outmate.services.deps import (
    get_queue_service,
    get_service,
...
from outmate.services.utils import initialize_services, initialize_settings_service, teardown_services
from outmate.utils.mcp_cleanup import cleanup_mcp_sessions
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            await logger.adebug(f"Starting Outmate v{version}...")
        else:
            await logger.adebug("Starting Outmate...")
        ...
        try:
            ...
            if "outmate migration --fix" not in str(exc):
            ...
            from outmate.__main__ import get_number_of_workers
            from outmate.cli.progress import create_outmate_shutdown_progress

            log_level = os.getenv("OUTMATE_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_outmate_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from outmate.api.v1.mcp import stop_streamable_http_manager
                    from outmate.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Outmate shutdown complete")
...
def create_app():
    ...
    if prome_port_str := os.environ.get("OUTMATE_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from outmate.api.v1 import mcp_router
    ...
    # Discover and register additional routers from plugins (outmate.plugins entry-point)
