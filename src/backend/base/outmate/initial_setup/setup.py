...
from outmate.initial_setup.constants import (
...
from outmate.services.database.models.folder.constants import (
    DEFAULT_FOLDER_DESCRIPTION,
    DEFAULT_FOLDER_NAME,
    LEGACY_FOLDER_NAMES,
)
from outmate.services.database.models.folder.model import Folder, FolderCreate, FolderRead
...
async def get_or_create_assistant_folder(session, user_id: UUID):
    """Create or get the OutmateAssistant folder for a specific user.
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    """Create or update agentic flows in the OutmateAssistant folder for a user.

    This function is called on user login to ensure that all agentic flows
    are present and up-to-date in the user's OutmateAssistant folder.
    ...
    try:
        # Get or create the OutmateAssistant folder
...
async def load_flows_from_directory() -> None:
    """On Outmate startup, this loads all flows from the directory specified in the settings.
