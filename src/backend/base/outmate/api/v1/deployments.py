...
from outmate.api.utils import CurrentActiveUser, DbSession, DbSessionReadOnly
from outmate.api.v1.schemas.deployments import (
...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="OutmateDB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="OutmateDB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="OutmateDB deployment UUID (`deployment.id`)."),
]


# API provider-context contract matrix:
# - Query/path ``provider_id`` is a OutmateDB UUID referencing deployment_provider_account.
...
# - Deployment-scoped routes derive provider context from persisted Outmate relationships.
...
async def list_deployment_types(
    ...
    """List deployment types for the selected Outmate provider-account UUID."""
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Not implemented.")
...
async def list_deployments(
    ...
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional Outmate flow version ids (pass as repeated query params, "
    ...
    """List deployments for the selected Outmate provider-account UUID."""
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Not implemented.")
...
async def create_deployment_execution(
    ...
    """Create a deployment execution for OutmateDB deployment/provider identifiers."""
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Not implemented.")
...
async def get_deployment_execution(
    ...
    """Get deployment execution status by provider-owned execution id and Outmate provider id."""
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Not implemented.")
