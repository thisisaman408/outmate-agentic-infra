...
from outmate.api.v1.mcp import ResponseNoOp
from outmate.api.v1.mcp_utils import (
...
from outmate.services.auth.constants import AUTO_LOGIN_WARNING
from outmate.services.auth.mcp_encryption import decrypt_auth_settings, encrypt_auth_settings
from outmate.services.database.models import Flow, Folder
from outmate.services.database.models.api_key.crud import check_key, create_api_key
from outmate.services.database.models.api_key.model import ApiKey, ApiKeyCreate
from outmate.services.database.models.user.crud import get_user_by_username
...
async def verify_project_auth_conditional(
    ...
    async with session_scope() as session:
        ...
        from outmate.services.auth.utils import get_current_user_mcp
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add OutmateAPI key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a OutmateAPI key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    outmate.api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", outmate.api_key])
        ...
        # Remove stale entries that point to the same OutmateURLs (e.g. after the project is renamed)
...
class ProjectMCPServer:
    def __init__(self, project_id: UUID):
        ...
        self.server = Server(f"Outmate MCP-project-{project_id}")
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Outmate host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
