...
* **Outmate managed (UUID)** -- ``id``, ``provider_id``, ``project_id``,
  ``deployment_id``. These reference rows in the outmate_database.
...
are defined by the provider adapter. Outmate forwards them without
...
This coupling is intentional -- these schemas carry no Outmate managed
...
class DeploymentProviderAccountCreateRequest(BaseModel):
    ...
    provider_tenant_id: NonEmptyStr | None = Field(
        default=None,
        description="Provider-owned tenant/organization id. Outmate persists this opaque value.",
    )
    provider_key: NonEmptyStr = Field(description="Deployment provider key.")
    provider_url: NonEmptyStr = Field(
        description="Provider service URL persisted in OutmateDB for provider-account resolution.",
    )
    api_key: SecretStr = Field(
        min_length=1,
        description=(
            "Provider credential material. Stored by Outmate as secret data and never returned in read responses."
        ),
    )
...
class DeploymentProviderAccountGetResponse(BaseModel):
    id: UUID = Field(description="OutmateDB provider-account UUID (`deployment_provider_account.id`).")
    provider_tenant_id: str | None = Field(
        default=None,
        description="Provider-owned tenant/organization identifier persisted as opaque text.",
    )
    provider_key: str = Field(description="Provider adapter key used by Outmate")
    provider_url: str = Field(description="Provider service URL persisted in OutmateDB.")
    created_at: datetime | None = Field(default=None, description="OutmateDB row creation timestamp.")
    updated_at: datetime | None = Field(default=None, description="OutmateDB row update timestamp.")
...
class _DeploymentResponseBase(BaseModel):
    ...
    id: UUID = Field(description="OutmateDB deployment UUID.")
    name: str
...
class FlowVersionsAttach(BaseModel):
    ...
    ids: list[str] = Field(
        min_length=1,
        description="Outmate flow version ids to attach to the deployment.",
    )
...
class FlowVersionsPatch(BaseModel):
    ...
    add: list[str] | None = Field(
        None,
        description="Outmate flow version ids to attach to the deployment. Omit to leave unchanged.",
    )
    remove: list[str] | None = Field(
        None,
        description="Outmate flow version ids to detach from the deployment. Omit to leave unchanged.",
    )
...
class DeploymentCreateRequest(BaseModel):
    ...
    provider_id: UUID = Field(description="OutmateDB provider-account UUID (`deployment_provider_account.id`).")
    spec: _StrictBaseDeploymentData = Field(description="Deployment metadata (service-layer schema, no ID fields).")
    project_id: UUID | None = Field(
        default=None,
        description="OutmateDB project id to persist the deployment under. Defaults to user's Starter Project.",
    )
...
class ExecutionCreateRequest(BaseModel):
    ...
    provider_id: UUID = Field(
        description=(
            "OutmateDB provider-account UUID (`deployment_provider_account.id`). "
            "Included alongside deployment_id to allow provider routing without an extra DB lookup."
        ),
    )
    deployment_id: UUID = Field(description="OutmateDB deployment UUID.")
    provider_data: dict[str, Any] | None = Field(
        default=None,
