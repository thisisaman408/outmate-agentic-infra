...
from outmate.api.v1.chat import build_flow_and_stream
from outmate.memory import aadd_messagetables
from outmate.schema.properties import Properties
from outmate.services.auth.utils import get_current_user_for_websocket
...
from outmate.services.deps import get_variable_service, session_scope
from outmate.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
        ...
        await websocket.send_json(
            {
                "type": "error",
                "code": "Outmate auth",
                "message": "You must pass a valid Outmate token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "outmate_voice_mode.config":
                            await logger.ainfo(f"outmate_voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "outmate_elevenlabs.config":
                            await logger.ainfo(f"outmate_elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "outmate_elevenlabs.config":
                            await logger.ainfo(f"outmate_elevenlabs.config {event}")
