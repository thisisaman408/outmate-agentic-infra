"""Utilities for auto-configuring the OutmateAgentic MCP server."""
...
from outmate.api.v2.mcp import get_server_list, update_server
from outmate.services.database.models.user.model import User
from outmate.services.deps import get_service, get_variable_service
from outmate.services.schema import ServiceType
from outmate.services.variable.constants import CREDENTIAL_TYPE, GENERIC_TYPE


async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the OutmateAgentic MCP server for all users.

    This function adds the outmate.agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring OutmateAgentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "outmate.agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "outmate.agentic.mcp"],
            "metadata": {
                "description": "OutmateAgentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "Outmate internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the OutmateAgentic MCP server from all users.

    This function removes the outmate.agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing OutmateAgentic MCP server from all users...")
