...
from outmate.services.utils import initialize_services
from outmate.utils.version import fetch_latest_version, get_version_info
from outmate.utils.version import is_pre_release as outmate_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="OutmateExecutor commands")
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("OutmateLOG_LEVEL") or "info").lower()
    os.environ["OutmateLOG_LEVEL"] = log_level
    ...
    progress = create_outmate_progress(verbose=verbose)

    # Step 0: Initializing Outmate
    ...
    if platform.system() == "Windows":
        ...
        from outmate.helpers.windows_postgres_helper import OUTMATE_DATABASE_URL, POSTGRESQL_PREFIXES
        ...
        with progress.step(6):
            # Use Gunicorn with OutmateUvicornWorker for non-Windows systems
            from outmate.server import OutmateApplication as outmate_application
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "outmate")
        'A new version of Outmate is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=outmate_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if outmate_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use outmate.utils.version to get the version info
    version_info = get_version_info()
    outmate_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= outmate_pre_release(outmate_version)  # Update pre-release status

    notice = build_version_notice(outmate_version, package_name)
...
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'outmate' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'outmate' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="OutmateLOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="OUTMATE_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set OutmateENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from outmate.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set OutmateAUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run outmate.api_key`")
            raise typer.Exit(1)
    ...
    async with session_scope() as session:
        from outmate.services.deps import get_auth_service
...
# command to copy the outmate_database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'outmate.db' and 'outmate_pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("outmate"))
    db_path = cache_dir / "outmate.db"
    pre_db_path = cache_dir / "outmate_pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from outmate.services.database.models.api_key.crud import create_api_key, delete_api_key
            from outmate.services.database.models.api_key.model import ApiKey, ApiKeyCreate
