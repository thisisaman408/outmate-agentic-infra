"""OutmateAssistant API router.

This module provides the HTTP endpoints for the OutmateAssistant.
...
from outmate.agentic.api.schemas import AssistantRequest
from outmate.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from outmate.agentic.services.flow_executor import execute_flow_file
from outmate.agentic.services.flow_types import (
    OUTMATE_ASSISTANT_FLOW,
    MAX_VALIDATION_RETRIES,
)
from outmate.agentic.services.provider_service import (
    DEFAULT_MODELS,
    PREFERRED_PROVIDERS,
...
async def check_assistant_config(
    ...
    """Check if the OutmateAssistant is properly configured.
...
async def assist(
    ...
    """Chat with the OutmateAssistant."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)

    logger.info(f"Executing {OUTMATE_ASSISTANT_FLOW} with {ctx.provider}/{ctx.model_name}")

    return await execute_flow_with_validation(
        flow_filename=OUTMATE_ASSISTANT_FLOW,
...
async def assist_stream(
    ...
    """Chat with the OutmateAssistant with streaming progress updates."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)
