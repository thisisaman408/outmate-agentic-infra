...
export default function FormcompletePage() {
    ...
    // Copilot automation agent results — last execute_search for 'form_complete'
    const copilotResult = useCoPilotAgentStore(s =>
        [...s.executionResults].reverse().find(r => r.module === 'form_complete' && r.status === 'success')
    )
    const copilotFilters = useCoPilotAgentStore(s => s.appliedFilters?.['form_complete'])
    const isHighlighted = useAgentHighlight('form_complete-filters-panel')

    const fetchSignals = async () => {
        try {
            setIsLoading(true)
            const data = await signalsApi.getSignals()
            const filtered = data.filter(s => s.category === 'formcomplete')
            setSignals(filtered)
        } catch (error) {
    ...
    return (
        <div id="form_complete-filters-panel" className={cn('space-y-6 p-6 transition-all duration-300', isHighlighted && 'ring-2 ring-primary ring-offset-2 rounded-xl')}>
            <div className="flex items-center justify-between">
                ...
                <Link href="/signals/new/custom?category=formcomplete">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-primary">
                                Automation Agent found {copilotResult.resultCount} form submission{copilotResult.resultCount !== 1 ? 's' : ''}
                        ...
                        <div className="mt-3 space-y-2">
                            {(copilotResult.results as any[]).slice(0, 10).map((item: any, i: number) => (
                                <div key={i} className="rounded-md bg-background border border-border/50 p-3 text-sm">
                                    <div className="font-medium">{item.email || item.name || item.company_name || `Submission ${i + 1}`}</div>
                                    {item.form_type && <div className="text-muted-foreground text-xs mt-0.5">Form: {item.form_type}</div>}
                                    {item.submitted_at && <div className="text-muted-foreground text-xs">Submitted: {item.submitted_at}</div>}
                                </div>
                            ))}
