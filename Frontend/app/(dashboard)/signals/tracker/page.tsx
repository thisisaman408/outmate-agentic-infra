...
export default function TrackerPage() {
    ...
    // Copilot automation agent results — last execute_search for 'trackers'
    const copilotResult = useCoPilotAgentStore(s =>
        [...s.executionResults].reverse().find(r => r.module === 'trackers' && r.status === 'success')
    )
    const copilotFilters = useCoPilotAgentStore(s => s.appliedFilters?.['trackers'])
    const isHighlighted = useAgentHighlight('trackers-filters-panel')
    ...
    return (
        <div id="trackers-filters-panel" className={cn('space-y-6 p-6 transition-all duration-300', isHighlighted && 'ring-2 ring-primary ring-offset-2 rounded-xl')}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-primary">
                                Automation Agent found {copilotResult.resultCount} tracker{copilotResult.resultCount !== 1 ? 's' : ''}
                                {copilotFilters?.keyword ? ` matching "${copilotFilters.keyword}"` : ''}
                            </span>
                        </div>
                        <Button
                            variant="ghost"
                        ...
                        <div className="mt-3 space-y-2">
                            {(copilotResult.results as any[]).slice(0, 10).map((item: any, i: number) => (
                                <div key={i} className="rounded-md bg-background border border-border/50 p-3 text-sm">
                                    <div className="font-medium">{item.name || `Tracker ${i + 1}`}</div>
                                    {item.category && <div className="text-muted-foreground text-xs mt-0.5">Category: {item.category}</div>}
                                    {item.status && <div className="text-muted-foreground text-xs">Status: {item.status}</div>}
                                </div>
                            ))}
