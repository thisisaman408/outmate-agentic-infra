...
INTEGRATION_CATALOG: List[Dict] = [
    ...
    {"slug": "hubspot", "name": "HubSpot", "category": "crm", "short_description": "Connect each user's own HubSpot portal for contacts, companies and deals", "auth_type": "oauth2", "is_active": True, "is_coming_soon": False, "credit_cost": "Free",
     "setup_steps": [
         "Click OAuth to authorize Outmate in your own HubSpot account",
         "Go to HubSpot → Settings → Integrations → Private Apps",
         "Choose the HubSpot portal you want Outmate to sync with",
         "Or paste your own HubSpot private app access token if you prefer BYOK",
     ],
     "features": ["User-owned OAuth connection", "Private app token fallback", "Contact & company sync", "Push from any table"],
    },
    ...
    {"slug": "outlook", "name": "Outlook / Office 365", "category": "email", "short_description": "Send and track emails through each user's Microsoft 365 mailbox via Microsoft Graph delegated OAuth", "auth_type": "oauth2", "is_active": True, "is_coming_soon": False, "credit_cost": "Free",
     "setup_steps": [
         "Outmate uses the Azure app registration configured for deployment as the OAuth client",
         "The user signs in with their own Microsoft 365 account and consents to delegated Graph permissions",
         "No Microsoft Graph access token or client secret should be pasted into Outmate by the user",
     ],
     "features": [
         "Send emails from the connected user's mailbox",
         "Read inbox and track replies with delegated Microsoft Graph permissions",
