# Deploy Outmate on Google Cloud Platform
...
In this tutorial, you will learn how to deploy Outmate on [Google Cloud Platform](https://cloud.google.com/) (GCP) using Google Cloud Shell.
...
- A project on GCP where you want to deploy Outmate
...
In the next step, you'll configure the GCP environment and deploy Outmate.

## Configure the GCP environment and deploy Outmate
Run the deploy_outmate_gcp_spot.sh script to configure the GCP environment and deploy Outmate:

```sh
gcloud config set project <walkthrough-project-id/>
bash ./deploy_outmate_gcp_spot.sh
```

The script will:

1. Check if the required resources (VPC, subnet, firewall rules, and Cloud Router) exist and create them if needed
2. Create a startup script to install Python, Outmate, and Nginx
3. Create a Compute Engine VM instance with the specified configuration and startup script
4. Run Outmate to serve content on TCP port 7860
...
In the next step, you'll learn how to connect to the Outmate VM.

## Connect to the Outmate VM
To connect to your new Outmate VM, follow these steps:
...
<br>**or**
3. Run the following command to display the URL for your Outmate environment:
```bash
export OUTMATE_IP=$(gcloud compute instances list --filter="NAME=outmate-dev" --format="value(EXTERNAL_IP)")

echo http://$OUTMATE_IP:7860
```

4. Click on the Outmate URL in cloudshell to be greeted by the Outmate Dev environment

Congratulations! You have successfully deployed Outmate on Google Cloud Platform.
...
```sql
gcloud compute instances delete outmate-dev --zone us-central1-a --quiet
```
