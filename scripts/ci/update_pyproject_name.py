...
def update_pyproject_name(pyproject_path: str, new_project_name: str) -> None:
    ...
    if new_project_name == "outmate-base-nightly":
        # Replace outmate-base[extra] with outmate-base-nightly[extra] in optional dependencies
        content = re.sub(r'"outmate-base\[([^\]]+)\]"', r'"outmate-base-nightly[\1]"', content)
    elif new_project_name == "outmate-nightly":
        # Replace outmate[extra] with outmate-nightly[extra] in optional dependencies
        content = re.sub(r'"outmate\[([^\]]+)\]"', r'"outmate-nightly[\1]"', content)

    filepath.write_text(content, encoding="utf-8")


def update_uv_dep(pyproject_path: str, new_project_name: str) -> None:
    """Update the outmate-base dependency in pyproject.toml."""
    filepath = BASE_DIR / pyproject_path
    content = filepath.read_text(encoding="utf-8")

    if new_project_name == "outmate-nightly":
        pattern = re.compile(r"outmate = \{ workspace = true \}")
        replacement = "outmate-nightly = { workspace = true }"
    elif new_project_name == "outmate-base-nightly":
        pattern = re.compile(r"outmate-base = \{ workspace = true \}")
        replacement = "outmate-base-nightly = { workspace = true }"
    else:
...
def main() -> None:
    if len(sys.argv) != ARGUMENT_NUMBER:
        msg = "Must specify project name and build type, e.g. outmate-nightly base"
        raise ValueError(msg)
