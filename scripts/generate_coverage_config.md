...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/outmate/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/outmate --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/outmate/components/**/*.py` (legacy components)
