...
Write-Host "Starting Outmate build and run process..." -ForegroundColor Green
...
if (Test-Path $envPath) {
    ...
    Write-Host "Outmate will use default configuration" -ForegroundColor Yellow
}
...
try {
    ...
    $targetDir = "src\backend\base\outmate\frontend"
...
# Step 4: Run Outmate
Write-Host "`nStep 4: Running Outmate..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Error running outmate: $_" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}

Write-Host "`nOutmate build and run process completed!" -ForegroundColor Green
Read-Host "Press Enter to exit"