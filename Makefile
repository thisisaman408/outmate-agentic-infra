.PHONY: all init format_backend format lint build run_backend dev help tests coverage clean_python_cache clean_npm_cache clean_frontend_build clean_all run_clic load_test_setup load_test_setup_basic load_test_list_flows load_test_run load_test_outmate_quick load_test_stress load_test_example load_test_clean load_test_remote_setup load_test_remote_run load_test_help docs docs_build docs_install
...
env ?= .env
open_browser ?= true
path = src/backend/base/outmate/frontend
workers ?= 1
async ?= true
lf ?= false
ff ?= true
...
clean_npm_cache:
	...
	$(call CLEAR_DIRS,src/frontend/node_modules src/frontend/build src/backend/base/outmate/frontend)
...
clean_frontend_build: ## clean frontend build artifacts to ensure fresh build
	...
	$(call CLEAR_DIRS,src/backend/base/outmate/frontend)
...
lint: install_backend ## run linters
	@uv run mypy --namespace-packages -p "outmate"
...
backend: setup_env install_backend ## run the backend in development mode
	...
	PYTHONPATH=.:src/backend/base OUTMATE_AUTO_LOGIN=true OUTMATE_SKIP_AUTH_AUTO_LOGIN=true uv run outmate run \
		--host 0.0.0.0 \
		--port $(port) \
		--dev \
		--backend-only \
...
ifdef base
	...
	make build_frontend
	make build_outmate_base args="$(args)"
endif
...
dockerfile_build_be: dockerfile_build
	...
	@$(DOCKER) build --rm \
		--build-arg outmate_IMAGE=outmate:${VERSION} \
		-f ${DOCKERFILE_BACKEND} \
		-t outmate_backend:${VERSION} .
...
alembic-revision: ## generate a new migration
	...
	cd src/backend/base/outmate/ && uv run alembic revision --autogenerate -m "$(message)"
...
alembic-downgrade: ## downgrade database by one version
	...
	cd src/backend/base/outmate/ && uv run alembic downgrade -1

alembic-current: ## show current revision
	@echo 'Showing current Alembic revision'
	cd src/backend/base/outmate/ && uv run alembic current

alembic-history: ## show migration history
	@echo 'Showing Alembic migration history'
	cd src/backend/base/outmate/ && uv run alembic history --verbose

alembic-check: ## check migration status
	@echo 'Running alembic check'
	cd src/backend/base/outmate/ && uv run alembic check

alembic-stamp: ## stamp the database with a specific revision
	@echo 'Stamping the database with revision $(revision)'
	cd src/backend/base/outmate/ && uv run alembic stamp $(revision)
...
patch: ## Update version across all projects. Usage: make patch v=1.5.0
	...
	\
	outmate_VERSION="$(v)"; \
	outmate_BASE_VERSION=$$(echo "$$outmate_VERSION" | sed -E 's/^[0-9]+\.(.*)$$/0.\1/'); \
	\
	echo "$(GREEN)outmate version: $$outmate_VERSION$(NC)"; \
	echo "$(GREEN)outmate-base version: $$outmate_BASE_VERSION$(NC)"; \
	\
	echo "$(GREEN)Updating main pyproject.toml...$(NC)"; \
	python -c "import re; fname='pyproject.toml'; txt=open(fname).read(); txt=re.sub(r'^version = \".*\"', 'version = \"$$outmate_VERSION\"', txt, flags=re.MULTILINE); txt=re.sub(r'\"outmate-base==.*\"', '\"outmate-base==$$outmate_BASE_VERSION\"', txt); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Updating outmate-base pyproject.toml...$(NC)"; \
	python -c "import re; fname='src/backend/base/pyproject.toml'; txt=open(fname).read(); txt=re.sub(r'^version = \".*\"', 'version = \"$$outmate_BASE_VERSION\"', txt, flags=re.MULTILINE); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Updating frontend package.json...$(NC)"; \
	python -c "import re; fname='src/frontend/package.json'; txt=open(fname).read(); txt=re.sub(r'\"version\": \".*\"', '\"version\": \"$$outmate_VERSION\"', txt); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Validating version changes...$(NC)"; \
	if ! grep -q "^version = \"$$outmate_VERSION\"" pyproject.toml; then echo "$(RED)✗ Main pyproject.toml version validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "\"outmate-base==$$outmate_BASE_VERSION\"" pyproject.toml; then echo "$(RED)✗ Main pyproject.toml outmate-base dependency validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "^version = \"$$outmate_BASE_VERSION\"" src/backend/base/pyproject.toml; then echo "$(RED)✗ outmate-base pyproject.toml version validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "\"version\": \"$$outmate_VERSION\"" src/frontend/package.json; then echo "$(RED)✗ Frontend package.json version validation failed$(NC)"; exit 1; fi; \
	...
	echo "  - pyproject.toml: $$outmate_VERSION"; \
	echo "  - src/backend/base/pyproject.toml: $$outmate_BASE_VERSION"; \
	echo "  - src/frontend/package.json: $$outmate_VERSION"; \
...
locust: ## run locust load tests (options: locust_users=10 locust_spawn_rate=1 locust_host=http://localhost:7860 locust_headless=true locust_time=300s locust_api_key=your-api-key locust_flow_id=your-flow-id locust_file=src/backend/tests/locust/locustfile.py locust_min_wait=2000 locust_max_wait=5000 locust_request_timeout=30.0)
	...
	export outmate_HOST=$(locust_host) && \
...
load_test_setup: ## Set up load test environment with starter project flows
	@echo "$(YELLOW)Setting up outmate load test environment$(NC)"
	@cd src/backend/tests/locust && uv run python outmate_setup_test.py --interactive
...
load_test_setup_basic: ## Set up load test environment with Basic Prompting flow
	...
	@cd src/backend/tests/locust && uv run python outmate_setup_test.py --flow "Basic Prompting" --save-credentials load_test_creds.json

load_test_list_flows: ## List available starter project flows
	@echo "$(YELLOW)Listing available starter project flows$(NC)"
	@cd src/backend/tests/locust && uv run python outmate_setup_test.py --list-flows
...
load_test_run: ## Run load test (automatically sets up if needed). Use FLOW_NAME="Flow Name" to specify flow
	...
	@if [ ! -f "src/backend/tests/locust/load_test_creds.json" ]; then \
		...
		if [ -z "$(FLOW_NAME)" ]; then \
			...
			cd src/backend/tests/locust && uv run python outmate_setup_test.py --flow "$(FLOW_NAME)" --save-credentials load_test_creds.json; \
		fi \
	fi
	@cd src/backend/tests/locust && \
	...
	uv run python outmate_run_load_test.py --headless --users 20 --duration 120 --no-start-outmate --html load_test_report.html --csv load_test_results

load_test_outmate_quick: ## Quick outmate load test (10 users, 30s) with HTML report (automatically sets up if needed). Use FLOW_NAME="Flow Name" to specify flow
	@echo "$(YELLOW)Running quick outmate load test with HTML report$(NC)"
	@if [ ! -f "src/backend/tests/locust/load_test_creds.json" ]; then \
		echo "$(BLUE)No credentials found. Running automatic setup...$(NC)"; \
		if [ -z "$(FLOW_NAME)" ]; then \
			echo "$(CYAN)Available flows:$(NC)"; \
			cd src/backend/tests/locust && uv run python outmate_setup_test.py --list-flows; \
			echo "$(RED)Please specify a flow: make load_test_outmate_quick FLOW_NAME=\"Basic Prompting\"$(NC)"; \
			exit 1; \
		else \
			echo "$(BLUE)Setting up with flow: $(FLOW_NAME)$(NC)"; \
			cd src/backend/tests/locust && uv run python outmate_setup_test.py --flow "$(FLOW_NAME)" --save-credentials load_test_creds.json; \
	...
	export API_KEY=$$(python -c "import json; print(json.load(open('load_test_creds.json'))['api_key'])") && \
	export FLOW_ID=$$(python -c "import json; print(json.load(open('load_test_creds.json'))['flow_id'])") && \
	uv run python outmate_run_load_test.py --headless --users 10 --duration 30 --no-start-outmate --html quick_test_report.html
...
load_test_stress: ## Stress test (100 users, 5 minutes) with comprehensive reporting (automatically sets up if needed). Use FLOW_NAME="Flow Name" to specify flow
	...
	export API_KEY=$$(python -c "import json; print(json.load(open('load_test_creds.json'))['api_key'])") && \
	export FLOW_ID=$$(python -c "import json; print(json.load(open('load_test_creds.json'))['flow_id'])") && \
	uv run python outmate_run_load_test.py --headless --users 100 --spawn-rate 5 --duration 300 --no-start-outmate --html stress_test_report.html --csv stress_test_results --shape ramp100
...
load_test_example: ## Run complete example workflow (setup + test + reports)
	...
	@cd src/backend/tests/locust && uv run python outmate_example_workflow.py --auto
...
load_test_remote_setup: ## Set up load test for remote instance (requires outmate_HOST)
	@if [ -z "$(outmate_HOST)" ]; then \
		echo "$(RED)Error: outmate_HOST environment variable required$(NC)"; \
		echo "$(YELLOW)Example: export outmate_HOST=https://your-remote-instance.com$(NC)"; \
		exit 1; \
	fi
	@echo "$(YELLOW)Setting up load test for remote instance: $(outmate_HOST)$(NC)"
	@cd src/backend/tests/locust && uv run python outmate_setup_test.py --host $(outmate_HOST) --flow "Basic Prompting" --save-credentials remote_test_creds.json
...
load_test_remote_run: ## Run load test against remote instance (requires prior setup)
	...
	@echo "$(YELLOW)Running load test against remote instance: $(outmate_HOST)$(NC)"
	@cd src/backend/tests/locust && \
	...
	uv run python outmate_run_load_test.py --host $(outmate_HOST) --no-start-outmate --headless --users 10 --spawn-rate 1 --duration 120 --html remote_test_report.html

load_test_help: ## Show detailed load testing help
	@echo "$(GREEN)outmate Enhanced Load Testing System$(NC)"
	@echo ""
	@echo "$(YELLOW)Quick Start (Local):$(NC)"
	@echo "  1. make load_test_setup_basic    # Set up with Basic Prompting flow"
	@echo "  2. make load_test_outmate_quick # Run quick outmate test"
	@echo "  3. Open quick_test_report.html  # View results"
	@echo ""
	@echo "$(YELLOW)Remote Testing:$(NC)"
	@echo "  1. export outmate_HOST=https://your-instance.com"
	...
	@echo "  load_test_outmate_quick - Quick outmate test (10 users, 30s)"
...
help_backend: ## show backend-specific commands
	...
	@echo "  $(GREEN)make build_outmate_base$(NC) - Build outmate-base package"
	@echo "  $(GREEN)make build_outmate$(NC)      - Build outmate package"
...
help_advanced: ## show advanced and miscellaneous commands
	...
	@echo "    This updates: pyproject.toml, outmate-base, frontend package.json"
	@echo ''
	...
	@echo "  $(GREEN)make publish_base$(NC)        - Publish outmate-base to PyPI"
	@echo "  $(GREEN)make publish_outmate$(NC)    - Publish outmate to PyPI"
	...
	@echo "  $(GREEN)make lock_base$(NC)           - Lock outmate-base dependencies"
	@echo "  $(GREEN)make lock_outmate$(NC)       - Lock outmate dependencies"
	@echo ''
