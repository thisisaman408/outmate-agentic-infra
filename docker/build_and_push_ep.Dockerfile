...
RUN --mount=type=cache,target=/root/.npm \
    ...
    && cp -r build /app/src/backend/outmate/frontend \
...
LABEL org.opencontainers.image.title=outmate
...
ENV OUTMATE_HOST=0.0.0.0
ENV OUTMATE_PORT=7860
ENV OUTMATE_EVENT_DELIVERY=polling

USER 1000
CMD ["python", "-m", "outmate", "run", "--host", "0.0.0.0", "--backend-only"]
